...
public class BorrowerTest {
   ...
   // Helper method to access internal borrowedMedia list via reflection
   @SuppressWarnings("unchecked")
   private List<Borrower.MediaRecord> getInternalBorrowedMedia() throws Exception {
       ...
       return (List<Borrower.MediaRecord>) borrowedMediaField.get(borrower);
   }
   ...
   @Test
   void removeBorrowRecord_UpdatesAndSavesFine_Success() throws Exception {
       ...
       getInternalBorrowedMedia().add(record);
   ...
   @Test
   void generateOverdueReport_WithOverdueItems() throws Exception {
       ...
       getInternalBorrowedMedia().add(borrower.new MediaRecord(book, LocalDate.now().minusDays(10), 1));
       // 5 days overdue for CD (Fine: 5 * 1.0 = 5.0)
       getInternalBorrowedMedia().add(borrower.new MediaRecord(cd, LocalDate.now().minusDays(5), 2));
   ...
   @Test
   void allMethods_Handle_ConnectionNull() throws Exception {
...
// =========================================================================
   ...
   @Test
   void loadBorrowedMedia_SkipsRecord_IfMediaDetailsMissing() throws Exception {
       // Setup: Ensure the internal list is clean (using reflection if needed, or assuming clear in BeforeEach)
       getInternalBorrowedMedia().clear();
...
// =========================================================================
    ...
    */
   @Test
   void removeBorrowRecord_SQLExceptionOnUpdate_RecordIsNotRemoved() throws Exception {
    ...
    */
   @Test
   void loadBorrowedMedia_BookDetailsQueryFails_RecordIsSkipped() throws Exception {
    ...
    */
   @Test
   void loadBorrowedMedia_CDDetailsQueryFails_RecordIsSkipped() throws Exception {
       // Setup: Ensure the internal list is clean
       getInternalBorrowedMedia().clear();
   ...
   void generateOverdueReport_MultipleOverdueItems_CalculatesFinesCorrectly() throws Exception {
       ...
       try (MockedStatic<BorrowingService> mocked = mockStatic(BorrowingService.class)) {
           ...
           // Check for book line - System.out.printf may use Arabic numerals depending on locale
           // Check for either Arabic numerals (١٠, ٥٫٠٠) or regular digits (10, 5.00)
           boolean hasBookTitle = cleaned.contains("العنوان: 'Clean Code'");
           boolean hasBookType = cleaned.contains("كتاب");
           boolean hasBookDays = cleaned.contains("أيام التأخير: ١٠") || cleaned.contains("أيام التأخير: 10");
           boolean hasBookFine = cleaned.contains("الغرامة: ٥٫٠٠") || cleaned.contains("الغرامة: 5.00");
           assertTrue(hasBookTitle && hasBookType && hasBookDays && hasBookFine,
                     "Book line missing. Output: " + cleaned);
           
           // Check for CD line - Check for either Arabic numerals or regular digits
           boolean hasCDTitle = cleaned.contains("العنوان: 'Best of Mozart'");
           boolean hasCDType = cleaned.contains("قرص مدمج");
           boolean hasCDDays = cleaned.contains("أيام التأخير: ٥") || cleaned.contains("أيام التأخير: 5");
           boolean hasCDFine = cleaned.contains("الغرامة: ٥٫٠٠") || cleaned.contains("الغرامة: 5.00");
           assertTrue(hasCDTitle && hasCDType && hasCDDays && hasCDFine,
                     "CD line missing. Output: " + cleaned);
           
           // Check for total fine - Check for either Arabic numerals or regular digits
           boolean hasTotalFine = cleaned.contains("إجمالي الغرامات المتأخرة: ١٠٫٠٠") || 
                                 cleaned.contains("إجمالي الغرامات المتأخرة: 10.00");
           assertTrue(hasTotalFine, "Total fine missing. Output: " + cleaned);
       }
   }
