...
public class AdminTest {
   ...
   void setUp() throws Exception {
       ...
       // استخدام constructor مع credentials تجريبية لتجنب تحميل .env
       mockEmailServer = new EmailServer("test@example.com", "test_password");
   ...
   public void testInitializeAdminTable_TableStructure() {
       ...
       try {
           ...
           while (resultSet.next()) {
               ...
               // Verify data type is not null (SQLite may return different type names)
               assertNotNull(dataType, "Data type should not be null for column " + columnName);
           }
           
           // Verify all expected columns are present
           for (String expectedCol : expectedColumns.keySet()) {
               assertTrue(foundColumns.contains(expectedCol), 
                   "Expected column " + expectedCol + " not found");
           }
   ...
   public void testInitializeAdminTable_Constraints() {
       ...
       if (conn != null) {
           try {
               ...
               // These constraints are verified by the database schema
   ...
   void testUnregisterUser_UserNotFound_ReturnsFalse() throws SQLException {
       ...
       try (MockedStatic<Admin> adminMock = Mockito.mockStatic(Admin.class, CALLS_REAL_METHODS)) {
           adminMock.when(Admin::connect).thenReturn(mockConn);
           // Setup for setAutoCommit(false) at start of method
   ...
   @Test
   public void testUnregisterUser_AutoCommitResetFailure2() {
       // Given - أي حالة
   ...
   public void testUnregisterUser_FailFinalDeletion() {
       ...
       // استخدام مستخدم غير موجود لاختبار السيناريو
       String username = "definitely_nonexistent_user_99999";
   ...
   public void testUnregisterUser_UserNotFound3() {
    ...
    public void testGetEmailServer() {
        EmailServer srv = Admin.getEmailServer();
        assertNotNull(srv, "getEmailServer should return a non-null EmailServer (was initialized in @BeforeEach)");
    }

    @Test
    public void testSetEmailServer_WithNull() {
        EmailServer original = Admin.getEmailServer();
        Admin.setEmailServer(null);
        // After setting null, getEmailServer should return null or the field updated accordingly
        assertNull(Admin.getEmailServer(), "setEmailServer(null) should set the server to null");
        // restore original
        Admin.setEmailServer(original);
    }

    @Test
    public void testSetEmailServer_WithNewServer() {
        EmailServer old = Admin.getEmailServer();
        EmailServer newSrv = new EmailServer("new@test.com", "pw");
        Admin.setEmailServer(newSrv);
        assertSame(newSrv, Admin.getEmailServer(), "setEmailServer should update the stored EmailServer");
        // restore
        Admin.setEmailServer(old);
    }

    @Test
    public void testSendOverdueReminders_EmailNotifierNull() {
        // Save and null the notifier
        var originalNotifier = Admin.emailNotifier;
        Admin.emailNotifier = null;

        // Should not throw
        assertDoesNotThrow(() -> Admin.sendOverdueReminders(), "sendOverdueReminders() should not throw when emailNotifier is null");

        // restore
        Admin.emailNotifier = originalNotifier;
    }

    @Test
    public void testSendOverdueReminders_WithException() {
        // Mock Borrower.getUsersWithOverdueBooks to return a list with one user
        try (MockedStatic<Borrower> bMock = mockStatic(Borrower.class)) {
            Borrower.UserWithOverdueBooks u = mock(Borrower.UserWithOverdueBooks.class);
            when(u.getUsername()).thenReturn("u1");
            when(u.getOverdueCount()).thenReturn(2);
            bMock.when(Borrower::getUsersWithOverdueBooks).thenReturn(List.of(u));

            // Make emailNotifier throw when notify is called
            var originalNotifier = Admin.emailNotifier;
            EmailNotifier notifier = mock(EmailNotifier.class);
            doThrow(new RuntimeException("notify fail")).when(notifier).notify(any(), anyString());
            Admin.emailNotifier = notifier;

            // Should not propagate exception
            assertDoesNotThrow(() -> Admin.sendOverdueReminders());
    ...
    @Test
    public void testSendOverdueReminders_WithMultipleUsers() {
        try (MockedStatic<Borrower> bMock = mockStatic(Borrower.class)) {
            Borrower.UserWithOverdueBooks u1 = mock(Borrower.UserWithOverdueBooks.class);
            Borrower.UserWithOverdueBooks u2 = mock(Borrower.UserWithOverdueBooks.class);
            when(u1.getUsername()).thenReturn("u1");
            when(u1.getOverdueCount()).thenReturn(1);
            when(u2.getUsername()).thenReturn("u2");
            when(u2.getOverdueCount()).thenReturn(3);
            bMock.when(Borrower::getUsersWithOverdueBooks).thenReturn(List.of(u1, u2));

            // replace notifier with spy to verify notify calls
            var originalNotifier = Admin.emailNotifier;
            EmailNotifier spyNotifier = mock(EmailNotifier.class);
            Admin.emailNotifier = spyNotifier;

            Admin.sendOverdueReminders();

            verify(spyNotifier, times(2)).notify(any(User.class), anyString());
    ...
    @Test
    public void testSendOverdueReminders_EmptyList() {
        try (MockedStatic<Borrower> bMock = mockStatic(Borrower.class)) {
            bMock.when(Borrower::getUsersWithOverdueBooks).thenReturn(Collections.emptyList());
            var original = Admin.emailNotifier;
            Admin.emailNotifier = mock(EmailNotifier.class);

            assertDoesNotThrow(() -> Admin.sendOverdueReminders(), "Should handle empty overdue list without error");

            verify(Admin.emailNotifier, never()).notify(any(), anyString());
            Admin.emailNotifier = original;
        }
    }

    @Test
    public void testUnregisterUser_RollbackException() throws Exception {
        // simulate connection and prepare statements and then throw on rollback
        Connection c = mock(Connection.class);
        PreparedStatement check = mock(PreparedStatement.class);
        ResultSet rs = mock(ResultSet.class);
        PreparedStatement deleteBorrow = mock(PreparedStatement.class);
        PreparedStatement deleteFines = mock(PreparedStatement.class);
        PreparedStatement deleteUser = mock(PreparedStatement.class);

        // Setup for setAutoCommit(false) at start of method
        doNothing().when(c).setAutoCommit(false);
        ...
        when(c.prepareStatement(startsWith("DELETE FROM borrow_records"))).thenReturn(deleteBorrow);
        when(c.prepareStatement(startsWith("DELETE FROM user_fines"))).thenReturn(deleteFines);
        when(c.prepareStatement(startsWith("DELETE FROM users"))).thenReturn(deleteUser);

        // make deleteUser.executeUpdate throw SQLException so flow goes to rollback
        when(deleteUser.executeUpdate()).thenThrow(new SQLException("delete failed"));

        // make rollback throw as well
        doThrow(new SQLException("rollback failed")).when(c).rollback();

        try (MockedStatic<Admin> adminMock = mockStatic(Admin.class, CALLS_REAL_METHODS)) {
            adminMock.when(Admin::connect).thenReturn(c);
            boolean result = Admin.unregisterUser("someuser");
            assertFalse(result, "Should return false when deletion fails and rollback throws");
        }
    }

    @Test
    public void testUnregisterUser_AutoCommitResetFailure() throws Exception {
        // Simulate normal deletion but fail when resetting autoCommit
        Connection c = mock(Connection.class);
        PreparedStatement check = mock(PreparedStatement.class);
        ResultSet rs = mock(ResultSet.class);
        PreparedStatement deleteBorrow = mock(PreparedStatement.class);
        ...
        when(deleteUser.executeUpdate()).thenReturn(1);
        
        // Setup commit to succeed
        doNothing().when(c).commit();

        // make setAutoCommit(true) throw when final reset happens
        doThrow(new SQLException("reset failed")).when(c).setAutoCommit(true);

        try (MockedStatic<Admin> adminMock = mockStatic(Admin.class, CALLS_REAL_METHODS)) {
            adminMock.when(Admin::connect).thenReturn(c);
            // Should return true since deletion succeeds (affectedRows = 1)
            // The setAutoCommit failure in finally block doesn't affect the return value
            boolean result = Admin.unregisterUser("anyuser");
            assertTrue(result, "Should return true when deletion succeeds, even if setAutoCommit fails");
            // We verify that setAutoCommit(true) was attempted in finally block
            verify(c).setAutoCommit(true);
        }
    }

    @Test
    public void testUnregisterUser_AffectedRowsZero() throws Exception {
        Connection c = mock(Connection.class);
        PreparedStatement check = mock(PreparedStatement.class);
        ResultSet rs = mock(ResultSet.class);
        PreparedStatement deleteBorrow = mock(PreparedStatement.class);
        ...
        // Simulate deletion affecting 0 rows
        when(deleteUser.executeUpdate()).thenReturn(0);
        
        // Setup rollback to succeed
        doNothing().when(c).rollback();

        try (MockedStatic<Admin> adminMock = mockStatic(Admin.class, CALLS_REAL_METHODS)) {
            adminMock.when(Admin::connect).thenReturn(c);
            boolean result = Admin.unregisterUser("userzero");
            assertFalse(result, "If affectedRows == 0, unregisterUser should return false");
            verify(c, times(1)).rollback();
        }
    }

    @Test
    public void testInitializeAdminTable_SQLExceptionInExecute() throws Exception {
        // Simulate DriverManager connect and createStatement.execute throwing
        Connection c = mock(Connection.class);
        Statement stmt = mock(Statement.class);
        try (MockedStatic<Admin> adminMock = mockStatic(Admin.class, CALLS_REAL_METHODS)) {
            adminMock.when(Admin::connect).thenReturn(c);
            when(c.createStatement()).thenReturn(stmt);
            doThrow(new SQLException("execute failed")).when(stmt).execute(anyString());
            // Should not throw to caller; method catches and prints
            assertDoesNotThrow(() -> Admin.initializeAdminTable());
    ...
    public void testAdminConstructor() {
        Admin a = new Admin("theadmin", "hashval", "saltval");
        assertEquals("theadmin", a.getUsername());
        assertFalse(a.isLoggedIn(), "Constructor should set loggedIn false");
    }
