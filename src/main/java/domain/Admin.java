...
public class Admin extends User {
   // this is the email server for the admin
  private static EmailServer emailServer;
    // this is the email notifier for the admin
   public static EmailNotifier emailNotifier;

   // Static initializer to safely initialize email components
   static {
       try {
           emailServer = new EmailServer();
           emailNotifier = new EmailNotifier(emailServer);
       } catch (Exception e) {
           System.err.println("تحذير: فشل في تهيئة خادم البريد الإلكتروني: " + e.getMessage());
           System.err.println("سيتم تعطيل إرسال الإشعارات عبر البريد الإلكتروني.");
...
//
   ...
   public static void sendOverdueReminders() {
       ...
       if (emailNotifier == null) {
           System.out.println("تحذير: خادم البريد الإلكتروني غير متاح. لم يتم إرسال التذكيرات.");
           return;
       }
       ...
       for (Borrower.UserWithOverdueBooks userWithOverdueBooks : usersWithOverdueBooks) {
           ...
           try {
               ...
               System.err.println("فشل في إرسال البريد الإلكتروني إلى " + username + ": " + e.getMessage());
